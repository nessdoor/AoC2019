...
(define rebase ; move the relative base
  (lambda (ip m0)
    (set! relative-base (+ relative-base (resolver (+ ip 1) m0)))
    (+ ip 2)))

(define run-intcode ; run an intcode program
  (lambda (program input-port output-port (cc 'nocont))
    (define ric-rec
      (lambda (ip)
        (let* ([instruction (memr ip)]
          ...
          (unless (= 99 opcode) ; stop at opcode 99
            (ric-rec (case opcode
                            ((1 2) (alu opcode ip mode0 mode1 mode2))             ; arithmetic ops branch
                            ((3 4) (io opcode ip mode0 input-port output-port))   ; I/O branch
                            ((5 6) (branch opcode ip mode0 mode1))                ; jumps branch
                            ((7 8) (compare opcode ip mode0 mode1 mode2))         ; comparison branch
                            ((9) (rebase ip mode0))                               ; adjust relative base
                            (else (error "Unexpected opcode: " opcode))))))))

    (set! memory-vect program)
    (set! exit-cont cc)
    (ric-rec 0)
